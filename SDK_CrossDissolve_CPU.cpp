	...
	PF_LayerDef* output)
{
	...
	out_data->out_flags2 |= PF_OutFlag2_PRESERVES_FULLY_OPAQUE_PIXELS | PF_OutFlag2_SUPPORTS_SMART_RENDER;
	...
	PF_LayerDef* output)
{
		...
		++y, outgoingData += outgoing->rowbytes, incomingData += incoming->rowbytes, destData += dest->rowbytes)
	{
		...
		//frame_data.clear(); // Clear frame_data after processing each row
	}
